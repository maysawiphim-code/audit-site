...
def check_data(sheet: Sheet, rep: Report):
    ...
    for t in D["tables"]:
        ...
        blocks = []
        for r in rows:
            if r["block"] and r["block"] not in blocks:
                blocks.append(r["block"])
        rep.structure.append({
            "คอลัมน์ป้ายกำกับ": f'{col_name(t["labelcols"][0])}–{col_name(t["labelcols"][1])}' if t["labelcols"] else "-",
            "กลุ่มคอลัมน์": " · ".join(g["title"] for g in t["groups"]),
            "คอลัมน์ค่า": " · ".join(sorted({k for g in t["groups"] for _, k in g["keys"]})),
            "บล็อกข้อมูล": " · ".join(blocks),
            "จำนวนแถวข้อมูล": len(rows),
            "แถวยอดรวม": sum(1 for r in rows if r["is_total"]),
        })
        for gr in t["groups"]:
...
def audit(data: bytes, filename: str) -> Report:
    ...
    if data_sheet is not None and not rep.key_totals:
        rep.issues.append(Issue(rep.data_name, "", "bad", "อ่านโครงตารางในชีต data ไม่ได้",
